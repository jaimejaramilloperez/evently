    ...
    : IQueryHandler<GetEventStatisticsQuery, EventStatisticsResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        CommandDefinition command = new(
            commandText: sql,
            parameters: new { EventId = request.EventId },
            cancellationToken: cancellationToken);

        EventStatisticsResponse? eventStatistics = await connection.QuerySingleOrDefaultAsync<EventStatisticsResponse>(command);
